...
{
    puts("mre-generate  - a program to display output of recurrent neural "
            "networks");
    puts("");
    puts("Usage: mre-generate [-s seed] [-n length] [-i index] mre-file "
            "gn-file");
    puts("Usage: mre-generate [-v] [-h]");
    puts("");
    ...
    puts("First, mre-generate reads the mre-file (ex: mre.dat) generated by "
            "mre-learn and gn-file (ex: gn.dat) generated by gn-learn in order "
            "to setup model parameters. Next, it displays output of a network "
            "up to given length with respect to the initial state "
            "corresponding to given index.");
}

static void display_version (void)
{
    printf("mre-generate version %s\n", TO_STRING(VERSION));
}
    ...
    while ((opt = getopt(argc, argv, "s:n:i:vh")) != -1) {
        switch (opt) {
            ...
            case 'h':
                ...
                exit(EXIT_SUCCESS);
            default: /* '?' */
                fprintf(stderr, "Try `mre-generate -h' for more "
                        "information.\n");
                exit(EXIT_SUCCESS);
        }
    }
