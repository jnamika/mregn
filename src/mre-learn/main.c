...
{
    puts("mre-learn  - an implementation of a gradient-based learning "
            "algorithm for mixture of RNN experts focusing on the problem "
            "of time-series prediction and generation");
    puts("");
    puts("Usage: mre-learn [-s seed] [-z experts] [-n neurons] "
            "[-t time-constant] [-d feedback-delay] "
            "[-k gate-prior-distribution] [-e epochs] [-l log-interval] "
            "[-x learning-rate] [-m momentum] [-a] [-g gamma] "
            "[-p prior-strength] [-i input-file] [-o output-file] "
            "[-c config-file] [-V] [target-file ...]");
    puts("Usage: mre-learn [-v] [-h]");
    puts("");
    ...
    puts("-z experts");
    puts("    Number of experts contained in the mixture of RNN experts. "
            "Default is " TO_STRING(EXPERT_NUM) ".");
    puts("-n neurons");
    puts("    Number of context neurons in each expert network. "
    ...
    puts("-k gate-prior-distribution");
    puts("    Type of prior distribution for gate opening values");
    puts("        0: No distribution");
    puts("        1: Gauss distribution (Default)");
    puts("        2: Cauchy distribution");
    puts("-e epochs");
    ...
    puts("-g");
    puts("    Control parameter for learning of gate opening values. "
            "Default is " TO_STRING(GAMMA) ".");
    ...
    puts("-i input-file");
    puts("    The input file `input-file' being generated by mre-learn is used "
            "for the initialization of model parameters. If there are no "
            "target-files named on the command line, mre-learn restarts "
            "training at the end of previous trial. Otherwise mre-learn "
    ...
    puts("mre-learn first reads parameters from the command line, or from a "
            "configuration file. Next, mre-learn proceeds to read each "
            "target-file which contains training examples. If there are no "
            "target-files named on the command line, mre-learn reads the "
...
static void init_parameters (struct general_parameters *gp)
{
    ...
    gp->mp.gamma = GAMMA;
    gp->mp.gate_prior_distribution = GATE_PRIOR_DISTRIBUTION;
    ...
    gp->iop.gate_filename = salloc(NULL, GATE_FILENAME);
    ...
    gp->iop.interval_for_gate_file = default_interval;
...
static void set_expert_num (const char *opt, struct general_parameters *gp)
{
    gp->mp.expert_num = atoi(opt);
}
...
static void set_fixed_gate (const char *opt, struct general_parameters *gp)
{
    gp->mp.fixed_gate = 1;
}
...
static void set_gamma (const char *opt, struct general_parameters *gp)
{
    gp->mp.gamma = atof(opt);
}

static void set_gate_prior_distribution (
        const char *opt, struct general_parameters *gp)
{
    gp->mp.gate_prior_distribution = atoi(opt);
}
...
static void set_gate_file (const char *opt, struct general_parameters *gp)
{
    gp->iop.gate_filename = salloc(gp->iop.gate_filename, opt);
}
...
#define SET_DEFAULT_VALUE_OF_PRINT_INTERVAL(OPT) \
    do { \
        ...
        SET_DEFAULT_VALUE_OF_PRINT_INTERVAL_I(gate_file,OPT); \
...
GEN_PRINT_INTERVAL_SETTER(gate_file)
...
static struct option_information {
    ...
    ENTRY_PRINT_INTERVAL_SETTER(gate_file),
...
{
    int opt;
    while ((opt = getopt(argc, argv, "s:z:n:t:d:k:e:l:x:m:ag:p:f:i:o:c:Vvh"))
            != -1) {
        switch (opt) {
            case 's':
                set_seed(optarg, gp);
            ...
            case 'd':
                ...
                break;
            case 'k':
                set_gate_prior_distribution(optarg, gp);
                break;
            case 'e':
            ...
            case 'h':
                ...
                exit(EXIT_SUCCESS);
            default: /* '?' */
                fprintf(stderr, "Try `mre-learn -h' for more information.\n");
                exit(EXIT_SUCCESS);
        }
    }
}
        ...
        const struct target_reader *t_reader)
{
    ...
    if (gp->mp.expert_num <= 0) {
        print_error_msg("number of experts must be greater than zero.");
        exit(EXIT_FAILURE);
    }
    ...
    if (gp->mp.gamma <= 0) {
        print_error_msg("`gamma' not in valid range: x > 0 (integer)");
        exit(EXIT_FAILURE);
    }
    if (gp->mp.gate_prior_distribution < 0 &&
            gp->mp.gate_prior_distribution > 2) {
        print_error_msg("type of prior distribution for gate opening values "
                "must be 0(no distribution), 1(gauss distribution), or "
